    ...
    <div style="max-width:600px;margin:0 auto;padding:40px 20px;">
      <div style="background:linear-gradient(135deg,#1e293b,#0f172a);border:1px solid #334155;border-radius:12px;overflow:hidden;">
        <div style="background:linear-gradient(135deg,#f59e0b,#d97706);padding:24px 32px;text-align:center;">
          <h1 style="margin:0;font-size:24px;color:#0f172a;font-weight:800;">Thumbnail<span style="color:#1e293b;">AI</span></h1>
        </div>
        <div style="padding:32px;">${content}</div>
        <div style="padding:20px 32px;border-top:1px solid #334155;text-align:center;">
          <p style="margin:0;font-size:12px;color:#64748b;">&copy; ${new Date().getFullYear()} ThumbnailAI. All rights reserved.</p>
        </div>
      </div>
    </div>
  </body>
...
export function sendPremiumEmail(to, name, dashboardUrl) {
  const html = emailWrapper(`
    ...
    <h2 style="margin:0 0 16px;color:#f59e0b;font-size:22px;text-align:center;">Welcome to Premium! 🎉</h2>
    <p style="color:#e2e8f0;font-size:15px;line-height:1.6;">Hi <strong>${name}</strong>,</p>
    <p style="color:#cbd5e1;font-size:15px;line-height:1.6;">Your payment was successful and your account has been upgraded to <strong style="color:#f59e0b;">Premium</strong>!</p>
    ...
    <div style="text-align:center;margin-top:28px;">
      <a href="${dashboardUrl}" style="display:inline-block;background:#f59e0b;color:#0f172a;font-size:15px;font-weight:700;padding:12px 32px;border-radius:8px;text-decoration:none;">Go to Dashboard →</a>
    </div>
  `);
  sendEmailAsync(to, "🎉 Premium Activated — Welcome to ThumbnailAI Premium!", html).catch(() => {});
}

export function sendThumbnailReadyEmail(to, name, prompt, imageUrl, dashboardUrl) {
  const html = emailWrapper(`
    <h2 style="margin:0 0 16px;color:#e2e8f0;font-size:22px;text-align:center;">Your Thumbnail is Ready! 🎨</h2>
    <p style="color:#e2e8f0;font-size:15px;line-height:1.6;">Hi <strong>${name}</strong>,</p>
    <p style="color:#cbd5e1;font-size:15px;line-height:1.6;">Your AI-generated thumbnail has been created successfully!</p>
    ...
    <div style="text-align:center;margin-top:28px;">
      <a href="${dashboardUrl}" style="display:inline-block;background:#f59e0b;color:#0f172a;font-size:15px;font-weight:700;padding:12px 32px;border-radius:8px;text-decoration:none;">View in Dashboard →</a>
    </div>
  `);
